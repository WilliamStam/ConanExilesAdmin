...
class blacklist extends _ {
	...
	function get($ID, $options = array()) {
		...
		if (count($result)) {
			...
			$return = parent::dbStructure("blacklist_ip");
		}
	...
	public function getData($where = "", $orderby = "", $limit = "", $options = array()) {
		...
		$result = $f3->get("DB")->exec("
			 SELECT DISTINCT blacklist_ip.*, system_users.fullname as user_name
			FROM blacklist_ip left join system_users ON system_users.ID = blacklist_ip.uID
			$where
			$orderby
			$limit;
		", $args, $ttl);
	...
	public static function _save($ID, $values = array()) {
		...
		$a = new \DB\SQL\Mapper($f3->get("DB"), "blacklist_ip");
		$a->load("ID='$ID'");
